...
func cmdClient(bindAddress, remoteAddress string, appCtx context.Context, cancel context.CancelFunc) int {
...
func cmdServer(bindAddress, remoteAddress string, appCtx context.Context, cancel context.CancelFunc) int {
	psk, err := simpleGetPSK()
	if err != nil {
		log.Printf("can't get PSK: %v", err)
		return 2
...
func run(argsStr string, appCtx context.Context, cancel context.CancelFunc) int {
	fields := strings.Fields(argsStr)
	flag.CommandLine.Parse(fields)
	args := flag.Args()
	...
	case 3:
		...
		case "server":
			// appCtx, cancel := signal.NotifyContext(context.Background(), syscall.SIGINT, syscall.SIGTERM)
			return cmdServer(args[1], args[2], appCtx, cancel)
			// return cmdServer(args[1], args[2])
		case "client":
			return cmdClient(args[1], args[2], appCtx, cancel)
		}
	}
	switch args[0] {
	case "hoppingclient":
...
var cancellations = make(map[int]*context.CancelFunc)
...
func stopDtlspipe(cancelID int) {
	threadMutex.Lock()
	cancel, ok := cancellations[int(cancelID)]
	if !ok {
		fmt.Println("error: cancel id %c, not found", cancelID)
		return
	}
	(*cancel)()
	threadMutex.Unlock()
